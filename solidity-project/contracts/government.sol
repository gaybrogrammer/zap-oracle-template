...
function create_government(uint money, string memory leader, uint parliamentary_seats, bytes32[3] memory leaders, bool tyrannical, uint private_money) public returns(Government memory){
uint id = governments.push(Government(money,leader, parliamentary_seats, leaders,  tyrannical, private_money)) - 1;
...
function presidential_election(string[] memory candidates,uint government_id) public{
    ...
    require(gov.tyrannical == false);
uint winner = randMod(candidates.length);
gov.leader = candidates[winner];

}
function parlimentary_election(bytes32[3] memory candidates,uint seat,uint government_id) public{
    Government storage gov = governments[government_id];
    require(gov.tyrannical == false);
uint winner = randMod(candidates.length);
gov.leaders[seat] = candidates[winner];
}
function tyrannical_election(string memory candidate,uint government_id) public{
    Government storage gov = governments[government_id];
require(gov.tyrannical == true);
